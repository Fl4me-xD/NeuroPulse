...
=============================
Runs the Edge Emulator simulation loop and streams live neuromorphic
metrics to the React frontend over WebSockets.
...
------------------
    ...
    Broadcasts a JSON frame every ~100 ms:
    {
        ...
        "nodes": {
            "pipeline_stress": {
                "sensor_value":               0.182,
                "membrane_potential":         0.314,
                "spike_fired":                false,
                "estimated_power_saved_pct":  94.3
            },
            ...
        },
        "network": {
            "any_spike":              false,
            "total_spikes_this_tick": 0,
            "overall_spike_rate":     0.057
        }
    }
...
--------------
    POST /trigger-anomaly
        Body: { "magnitude": 3.5, "duration_ticks": 20 }
        Injects a high-current shock into all sensor nodes, saturating
        their membrane potentials and triggering cascading spike trains.

    GET  /logs?node_id=pipeline_stress&limit=200
    GET  /anomalies?resolved=false&limit=50
    GET  /power-summary
    GET  /network-summary
    POST /reset
"""
